...
In the terminal we get an error message that it is not installed so we use 
```
...
Because we want to work in this directory we first use `ls` to see which lists are available.
The file we have is called `Data-Science-Toolkits-and-Architectures-group-1`. 
Therefore, we use the code.
```
cd Data-Science-Toolkits-and-Architectures-group-1
```
