...
def plot_confusion_matrix(classifier, test_data, test_labels):
    ...
    ConfusionMatrixDisplay(cm, display_labels=range(config.num_classes)).plot(cmap="Blues", ax=ax) 
...
def predict_image_label(model, image_path):
    ...
    img = keras_image.load_img(image_path, target_size=input_shape, color_mode=color_mode)
